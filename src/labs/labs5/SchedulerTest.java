...
class Timestamp<T> implements Comparable<Timestamp<?>>{
    ...
    public Timestamp(LocalDateTime time, T elem) {
        this.time = time;
        this.elem = elem;
    }
...
class Scheduler<T>{
    ...
    public List<Timestamp<T>>getAll(LocalDateTime begin, LocalDateTime end){
        Timestamp<String>beginTime=new Timestamp<>(begin,"begin");
        Timestamp<String>endTime=new Timestamp<>(end,"end");
        return list.stream().filter(t->t.compareTo(beginTime)>0 && t.compareTo(endTime)<0)
